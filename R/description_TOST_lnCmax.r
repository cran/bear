...
cat("**Interpretation:\n")
cat(paste("Ho: Theta < ",formatC(theta1,format="f",digits=5)," or  Theta > ",formatC(theta2,format="f",digits=3),"\n",sep=""))
cat(paste("Ha: ",formatC(theta1,format="f",digits=5)," < or = Theta < or = ",formatC(theta2,format="f",digits=3),"\n",sep=""))
cat("where Theta = Mean_Test/Mean_Ref.\n")
cat("Because at least one of P values is greater than 0.05,\nwe can not reject the null hypothesis (Ho).\n")
