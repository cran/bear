###
### Aceto revised NCA codes for lambda_z (WinNolin); Aceto? really? got to be mistake... YJ
### other methods like this: NCAselect(), aic(), TTT(), TTTAIC(), ARS(). 
###

TTTARS<-function(Dose, xaxis,yaxis,Totalplot,SingleRdata,SingleTdata,SingleRdata1,SingleTdata1, Tau, TlastD,SingleRdata0,SingleTdata0,  
...
###
          ...
          xxx1<-(R.split[[j]][(nrow(R.split[[j]])-n_TTT_ARS+1):nrow(R.split[[j]]),])$time-TlastD}  
          else{                           ### for single-dose; original whole dataset
          xx1<-R.split[[j]]$time
          xxx1<-(R.split[[j]][(nrow(R.split[[j]])-n_TTT_ARS+1):nrow(R.split[[j]]),])$time  ### selected data points for regression line
          }         
          yy1<-R.split[[j]]$conc
          yyy1<-(R.split[[j]][(nrow(R.split[[j]])-n_TTT_ARS+1):nrow(R.split[[j]]),])$conc
          if (replicated){
               main<-paste("[TTTARS] Subj.# ",subj[j]," [Period# ",prd[j],", Seq# ",seq[j]," - Ref.]",sep="")}
           else {
               main<-paste(c("[TTTARS] Subj.#", R.split[[j]]$subj[1],"- Ref."),collapse=" ")}
          if(multiple){
              ...
              #calculate AUC (0~INF)
               ...
               aucINF<-auc_ref[length(R.split[[j]][["conc"]])]+auc.infinity
                ...
                #calculate AUMC (0~INF)
                 ...
                 ###
                 datalog<-as.data.frame(R.split[[j]][(nrow(R.split[[j]])-n_TTT_ARS+1):nrow(R.split[[j]]),])
...
###
          ...
          xxx1<-(T.split[[j]][(nrow(T.split[[j]])-n_TTT_ARS+1):nrow(T.split[[j]]),])$time-TlastD}  
          else{                           ### for single-dose; original whole dataset
          xx1<-T.split[[j]]$time
          xxx1<-(T.split[[j]][(nrow(T.split[[j]])-n_TTT_ARS+1):nrow(T.split[[j]]),])$time  ### selected data points for regression line
          }         
          yy1<-T.split[[j]]$conc
          yyy1<-(T.split[[j]][(nrow(T.split[[j]])-n_TTT_ARS+1):nrow(T.split[[j]]),])$conc
          if (replicated){
               main<-paste("[TTTARS] Subj.# ",subj1[j]," [Period# ",prd1[j],", Seq# ",seq1[j]," - Test]",sep="")}
           else {
               main<-paste(c("[TTTARS] Subj.#", T.split[[j]]$subj[1],"- Test"),collapse=" ")}
          if(multiple){
             plot(xx1,yy1, log="y", axes=FALSE,xlim=range(0, 1.2*Tau), ylim=c(1e-3,1e+5),
             xlab=xaxis, ylab= paste(yaxis,"(as log10 scale)",sep=" "),     ## log="y" as semilog plot here (YJ)
             main=main,las=1, cex.lab = 1.2,cex.main = 0.8,pch=19,frame.plot=FALSE)   ### remove plot frame with'frame.plot=FALSE' here  -YJ
              ...
              #calculate AUC (0~INF)
               ...
               aucINF<-auc_test[length(T.split[[j]][["conc"]])]+auc.infinity
                ...
                #calculate AUMC (0~INF)
                 ...
                 ### save point selection here -YJ
                 ###
                 datalog<-as.data.frame(T.split[[j]][(nrow(T.split[[j]])-n_TTT_ARS+1):nrow(T.split[[j]]),])
 ...
 if (Demo){
   ...
   if(parallel){
      if(multiple){
        ...
        MultipleParaTTTARSoutput(sumindexR, sumindexT,R.split, T.split,keindex_ref,keindex_test,Dose,TotalData, Tau, TlastD)
      ...
      if (Demo){
       ...
       if(multiple){
        SingleRdata<-SingleRdata0
        SingleTdata<-SingleTdata0
        MultipleTTTARSoutput(sumindexR, sumindexT,R.split, T.split,keindex_ref,keindex_test,Dose,TotalData, Tau, TlastD)
